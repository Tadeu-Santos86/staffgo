...
function editarCandidatura(id, nomeAtual, whatsappAtual, experienciaAtual) {
  candidaturaEditandoId = id;

  const campoNome = document.getElementById("edit-nome");
  const campoWhatsApp = document.getElementById("edit-whatsapp");
  const campoExperiencia = document.getElementById("edit-experiencia");
  const modal = document.getElementById("modal-editar");

  if (!campoNome || !campoWhatsApp || !campoExperiencia || !modal) {
    alert("O modal de edição não está disponível no HTML atual.");
    return;
  }

  campoNome.value = nomeAtual || "";
  campoWhatsApp.value = whatsappAtual || "";
  campoExperiencia.value = experienciaAtual || "";

  modal.style.display = "block";
}

async function salvarEdicaoCandidatura() {
  if (!candidaturaEditandoId) {
    alert("Nenhuma candidatura selecionada para edição.");
    return;
  }

  const nome = document.getElementById("edit-nome")?.value.trim() || "";
  const whatsapp = document.getElementById("edit-whatsapp")?.value.trim() || "";
  const experiencia = document.getElementById("edit-experiencia")?.value.trim() || "";
  ...
  try {
    await updateDoc(doc(db, "Candidatos", candidaturaEditandoId), {
      Nome: nome,
      WhatsApp: whatsapp,
      Experiencia: experiencia
    });
